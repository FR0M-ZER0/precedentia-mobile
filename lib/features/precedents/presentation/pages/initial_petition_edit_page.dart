...
class _InitialPetitionEditPageState extends State<InitialPetitionEditPage> {
  ...
  late int _petitionId;
  bool _isLoading = false;

  static const String _fallbackMarkdown = '''# Petição Inicial
...
Descreva os fatos aqui.

---

## Do Direito

Fundamente juridicamente aqui.
...
''';
  ...
  void initState() {
    ...
    final content = widget.extra?['content'] as String? ?? _fallbackMarkdown;
    _petitionId = widget.extra?['id'] as int? ?? 0;
  ...
  Future<void> _submitEdit() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/generation/petitions/edit'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Petição atualizada com sucesso')),
          );
        }
      } else {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erro ao editar petição: $e'),
